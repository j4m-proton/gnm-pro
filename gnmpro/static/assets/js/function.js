...
$(document).ready(function () {
    ...
    function refreshCart() {
        $.ajax({
            url: "/cart/", // URL to fetch the updated cart content
            dataType: "html",
            success: function (response) {
                $("#cart-list").html($(response).find("#cart-list").html());
            },
            error: function (jqXHR, textStatus, errorThrown) {
                console.log("Error refreshing cart:", textStatus, errorThrown);
            }
        });
    }
...
$(document).ready(function() {
    ...
    $(".delete-product").on("click", function () {
        ...
        $.ajax({
            ...
            dataType: "json",
            beforeSend: function () {
                this_val.prop('disabled', true); // Disable the button
            },
            success: function (response) {
                console.log("Success callback triggered");
                console.log("Response Data:", response);

                // Update the cart list and item count
                $("#cart-list").html(response.data);
                $("#cart-dropdown").html(response.data2);
                $(".cart-items-count").text(response.totalcartitems);
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                ...
                this_val.prop('disabled', false); // Re-enable the button on error
            }
        });
    });
});
